...
struct MapViewDiscoverDetailed: UIViewRepresentable {
    ...
    class Coordinator: NSObject, MKMapViewDelegate {
        var parent: MapViewDiscoverDetailed

        init(_ parent: MapViewDiscoverDetailed) {
            self.parent = parent
        }
    ...
    func makeUIView(context: Context) -> MKMapView {
        if let mapView = MapViewDiscoverDetailed.mapViewStore[key] {
           return mapView
        }
        ...
        MapViewDiscoverDetailed.mapViewStore[key] = mapView
        return mapView
    }
